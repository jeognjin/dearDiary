...
<c:if test="${errors.FailToRegister}">
<script type="text/javascript">
alert("회원가입 실패. 다시 시도해주세요.");
</script>
</c:if>
<form action="${contextPath }/member/regist.do" method="post" class="joinForm">
      ...
      <div class="textForm">
        <input name="nickName" type="text" class="name" placeholder="닉네임<c:if test="${errors.nickName}">을 입력하세요.</c:if>">
        
      </div>
       <div class="textForm">
        <input name="email" type="text" class="email" placeholder="이메일<c:if test="${errors.email}">을 입력하세요.</c:if>">
      </div>
       <c:if test="${errors.nickNameOrIdDuplicated}"><p style="color: red;">ID 또는 닉네임이 중복되었습니다.</p></c:if>
