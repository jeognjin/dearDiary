...
<div id="slider">
 ...
 <c:if test="${errors.FailToChangeInfo}">
<script type="text/javascript">
alert("회원정보 변경 실패. 다시 시도해주세요.");
</script>
</c:if>         
<form action="${contextPath }/member/changeInfo.do" method="post" class="joinForm">
                                                                                     
      <h2>회원정보 변경</h2>
      <div class="textForm">
        <input name="memberId" type="text" class="id" value="${changeInfoId}" readonly>
      ...
      <div class="textForm">
       ...
       <div class="textForm">
        <input name="confirmPassword" type="password" class="pw" placeholder="비밀번호 확인<c:if test="${errors.confirmPassword}">을 입력하세요.</c:if><c:if test="${errors.notMatchPassword}">이 비밀번호와 동일하지 않습니다.</c:if>">     
      </div>
      
      <div class="textForm">
        <input name="nickName" type="text" class="name" value="${changeInfoNickname}" readonly>
      </div>
      
       <div class="textForm">
        <input name="email" type="text" class="email" placeholder="이메일<c:if test="${errors.email}">을 입력하세요.</c:if>">
      </div>
      <p style="color: brown; font-size: 14px;">ID와 닉네임은 변경 할 수 없습니다.</p>
      <p style="color: brown; font-size: 14px;">ID 또는 닉네임 변경을 원하시면 탈퇴 후 재가입해주세요.</p>
